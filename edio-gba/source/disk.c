...
u8 diskCmdSD(u8 cmd, u32 arg) {
    ...
    i = 0;
    for (;;) {
        if ((ed_sd_cmd_val() & 192) == 0)break;
        //if (ed_sd_cmd_val() != 0xff)break;
        //if ((ed_sd_cmd_val() & 192) != 0)break;
...
u8 diskInit() {
    ...
    ed_sd_speed(SD_SPD_LO);

    ed_sd_mode(SD_MODE8);

    for (i = 0; i < 40; i++)ed_sd_dat_wr(0xff);
...
u8 diskWrite(u32 sd_addr, void *src, u16 slen) {
    ...
    while (slen--) {
        ...
        src += 512;
        //ed_sd_mode(SD_MODE4);
        //for (i = 0; i < 256; i++)ed_sd_dat_wr16(*buff++);

        ed_sd_mode(SD_MODE2);
        for (i = 0; i < 4; i++) {
            //ed_sd_dat_wr16(crc16[i]);
