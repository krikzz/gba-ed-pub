...
u8 ed_sd_dma_rd(void *dst, int slen) {
    ...
    if (((u32) dst & 0xE000000) == 0x8000000)return ed_sd_dma_to_rom(dst, slen);
...
void ed_set_save_type(u8 save_type) {
    ...
    if (save_type != 0)ed_set_save_type(0);
...
void ed_set_ram_bank(u16 bank) {
    ...
    bank = bank == 3 ? ED_RAM_BNK_3 : bank == 2 ? ED_RAM_BNK_2 : bank == 1 ? ED_RAM_BNK_1 : ED_RAM_BNK_0;
...
void ed_reboot(u8 quick_boot) {
    ...
    ed_reg_wr(REG_CFG, 0);
    GBA_WAITCNT = 0;
    if (quick_boot)asm("swi 0x000000");
    asm("swi 0x26");
}
